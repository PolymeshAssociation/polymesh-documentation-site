...
[SDK documentation](/sdk-docs/) is recommendable.
...
- A Polymesh client has been [instantiated](/sdk-docs/classes/API/Client/Polymesh/#connect) by Alice so she can do the next actions:
...
Before creating the asset proper, Alice needs to [reserve](/sdk-docs/classes/API/Client/Assets/#reserveticker) the ACME ticker so that it is not squatted while the founders incorporate the company. Think of it on the same level as grabbing your `.com` domain as early as possible:
...
How do you get it back? Use [`assets.getTickerReservation`](/sdk-docs/classes/API/Client/Assets/#gettickerreservation):
