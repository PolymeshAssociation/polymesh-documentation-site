...
Carrying the tokens over follows the standard _lock and mint_ process. First, Ethereum tokens are locked permanently by an Ethereum smart contract. Then, native tokens are minted on the Polymesh side.
...
Relayers are members of the bridge multisig account owned by the _bridge controller_, and provided a quorum agree, POLY locked on Ethereum will be bridged to Polymesh.
