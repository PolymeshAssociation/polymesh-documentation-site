...
While Polymesh is a public-permissioned blockchain, [Polymesh Private]((../../polymesh-private/) is a variant of Polymesh designed to be deployed as a private blockchain between participating chain operators. There is exactly one Polymesh mainnet, whereas there can be many independent instances of Polymesh Private chains.

Polymesh Private is very similar to Polymesh, with the following exceptions:
  - Polymesh Private chains are expected to be run amongst a small set of peers on a private network.
  - Polymesh Private includes a [Confidential Asset](../../polymesh-private/confidential-assets/) module to allow private asset settlements between counterparties.
  - Polymesh Private does not involve the use of the [POLYX](../polyx/) token. While transactions can still have fees associated with them, these fees are paid in a token associated with the individual Polymesh Private instance which is not expected to have intrinsic value. Alternatively Polymesh Private transactions can be modified to have no fees.
  - Usage of Polymesh Private is governed by a license - see https://github.com/PolymeshAssociation/polymesh-private?tab=readme-ov-file#license for more details.
...
[Identity](../../primitives/identity/) is at the core of Polymesh. Polymesh implements a federated root of trust for onboarding all users, via permissioned Customer Due Diligence service providers. Every transaction in Polymesh is associated with an identity.
