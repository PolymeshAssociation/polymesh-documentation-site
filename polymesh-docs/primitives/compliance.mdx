...
The compliance manager, implemented in the base layer primitives of the Polymesh blockchain, provides a flexible framework to allow asset issuers to easily configure complex transfer rules based on claims that must be held by their investors.
...
The above diagram shows the available claim types. Some of these claim types are parametrised with additional information relevant to the claim, for example `Jurisdiction` includes the country code of the users jurisdiction.
...
There is an additional "special" claim not shown above which is treated differently, this is:
