...
Compliance rules can be enforced on the Token Studio's Attestations tab.
Add one or more attestations to any Polymesh Account. Only attestations written by approved attestation providers will be checked when a tokenholder is sending or receiving your security tokens.

![Attestations on Token Studio](../images/attestations_token_studio.png)
...
![Claims on Polymesh Portal](../images/claims_polymesh_portal.png)

Fill in the details of the claim like the target DID, scope, scope value and claim type then click on the 'Create' button and sign the transaction with your wallet. 

![Create new claim](../images/create_claim_polymesh_portal.png)
