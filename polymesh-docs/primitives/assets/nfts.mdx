...
- If the asset already exists, it must be of type `NonFungible` and the caller must have the proper permission for the asset. If the asset does not already exist, the `create_nft_collection` dispatchable will also create one non-fungible asset using the values passed as `nft_type`;
- This must be the first collection associated to the given `AssetId` (i.e. only one collection per Asset ID is allowed);
