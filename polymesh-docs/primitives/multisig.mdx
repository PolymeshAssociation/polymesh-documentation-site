...
To begin, we create three new keys, called `MULTISIG_SIGNER_1/2/3`. In practice these can be created and secured by three different parties.
...
In this tutorial, `POLY_ORG` is the Primary Key of our identity, so we use this key to submit the `createMultisig` transaction.
...
Note that the original Primary Key (`POLY_ORG`) is no longer attached to an Identity (since it was replaced by our new MultiSig). This means that any POLYX on the key will be locked until it is attached to a new CDD'ed Identity.
