...
there are what is known as [_corporate actions_ (CAs)][ca].
Examples of these include dividend payments, stock splits,
and ballot for the [_annual general meeting_ (AGM)][agm].
...
The function must be called by the asset's _corporate action agent_ (CAA).
...
For CAs, the checkpoint used is known as the _record date_,
...
Polymesh can handle this _withholding tax_ natively,
allowing you to specify how many % that should be withheld.
This % can either be specified generally for all investors
(the _default withholding tax_),
or customized to specific individual investors (_DID-specific withholding tax_).
...
or _target_, every investor holding any asset balance.
...
e.g., a) _"aye"_, b) _"aye with X amendment"_, and c) _"nay"_, to which weights can be assigned.
...
Optionally, a ballot can be configured with [_ranked-choice voting (RCV)_][rcv].
