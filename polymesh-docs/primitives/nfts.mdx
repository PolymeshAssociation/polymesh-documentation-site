...
The above specification means that an image can either be specified via the [Image URI](#image-uri) or via an `image` tag in the [Token URI](#token-uri). The recommended approach is to look for a dedicated [Image URI](#image-uri) first, and then fall back to the [Token URI](#token-uri).
