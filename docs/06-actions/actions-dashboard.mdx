...
You will be offered 10,000 STBL tokens at no cost. This faucet gives them away because STBL tokens are valueless.
...
The dividend distribution requires Alice's consent. Press `Accept` and sign the transaction to receive the funds in the default portfolio.
