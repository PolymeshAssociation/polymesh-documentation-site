...
So EVM support is not merely an "interface" onto PolkaVM — EVM bytecode runs in a separate VM (`revm`) from PolkaVM contracts. The `AllowEVMBytecode` config flag (below) toggles the `revm` execution path; it is `true` on all Polymesh networks. See the [Smart Contracts](/development/smart-contracts) conceptual guide for the compiler/tooling detail.
