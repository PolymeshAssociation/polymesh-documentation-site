...
While the main [Polymesh SDK](./) abstracts away most direct blockchain interactions, [`polymesh-types`](https://www.npmjs.com/package/@polymeshassociation/polymesh-types) serves developers who need to work at a lower level, typically using libraries like [`@polkadot/api`](../050-polkadot-api.mdx).
...
*   Developing applications using [`@polkadot/api`](../050-polkadot-api.mdx) directly against a Polymesh node.
...
This example shows how to initialize [`@polkadot/api`](../050-polkadot-api.mdx) for Polymesh using the `typesBundle`.

```typescript title="connect.ts"
import { ApiPromise, WsProvider } from '@polkadot/api';
// Import from the specific chain version bundle you need
// Usually, importing from 'bundle' is sufficient as it contains necessary overrides
...
The primary [`@polymeshassociation/polymesh-sdk`](https://www.npmjs.com/package/@polymeshassociation/polymesh-sdk) package uses [`polymesh-types`](https://www.npmjs.com/package/@polymeshassociation/polymesh-types) internally. It provides higher-level abstractions (Entities like `Asset`, `Identity`, `Portfolio`; Namespaces like `assets`, `identities`) that simplify common operations and hide much of the direct [`@polkadot/api`](../050-polkadot-api.mdx) interaction.
...
*   **Direct API Interaction:** [`@polkadot/api` Usage](../050-polkadot-api.mdx)
