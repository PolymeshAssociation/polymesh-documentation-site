...
- **API at Block State:** We get a specific instance of the API pinned to the state of the chain _at that block hash_ using `api.at(hash)`. This is essential because chain state can change between blocks, and querying events requires looking at the state corresponding to the block they occurred in.
...
async function processBlockEvents(hash: string): Promise<void> {
  ...
  try {
    ...
    console.log(
      `--- Processing Block #${blockNumber} (${hash.substring(0, 10)}...) ---`
    );
    ...
    events.forEach((record) => {
      ...
      if (api.events.balances.Transfer.is(event)) {
        ...
        if (extrinsicId !== undefined) {
          console.log(
            `     Context: Extrinsic #${extrinsicId} (${section}.${method}), Hash: ${extrinsicHash?.substring(0, 10)}...`
          );
        }
      }
...
const main = async () => {
  ...
  try {
    ...
    console.log(
      'Subscription active. Waiting for new blocks... (Press Ctrl+C to exit)'
    );
