...
<Tabs>
<TabItem value="local" label="Local Signing Manager (Development)" default>
...
async function main() {
  ...
  // Use the SDK...
  const signingAccount = await signingManager.getAccounts()[0];
...
<TabItem value="browser" label="Browser Extension (Web dApp)">
...
async function main() {
  // Ensure this code runs in a browser environment where the extension is present
  if (typeof window === 'undefined' || !window.injectedWeb3) {
    console.error('Polymesh Wallet extension not found.');
    return;
  }

  const appName = 'My Awesome Polymesh dApp';
  const signingManager = await BrowserExtensionSigningManager.create(appName);
  ...
  // Get available accounts from the extension (user must authorize)
  const accounts = await signingManager.getAccounts();
  if (!accounts.length) {
    console.error('No accounts authorized in the Polymesh Wallet extension.');
    await sdk.disconnect();
    return;
  }

  // Use the first authorized account for signing
  ...
  // Example: Reserve a ticker (will prompt user in extension)
  try {
    const ticker = 'MYDAPPTOKEN';
    const reserveTx = await sdk.assets.reserveTicker({ ticker });
    console.log('Reserving ticker...');
    await reserveTx.run();
    console.log(`Ticker ${ticker} reserved successfully!`);
  } catch (error) {
