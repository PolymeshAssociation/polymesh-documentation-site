...
The node operator role on Polymesh requires a permission to be assigned. All Operators satisfy selection criteria determined by Polymesh and be approved by the the Polymesh Governing Council.
...
1. Create a new unit file called `polymesh.service` in `/etc/systemd/system/` with the following content:
   ...
   Restart=always
   RestartSec=5
   ExecStart=/usr/local/bin/polymesh --chain mainnet --pruning archive --name <your-node-name> --operator
