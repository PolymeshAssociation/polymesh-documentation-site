...
- `8.0.0-mainnet-{debian|distroless}` and `latest-mainnet-{debian|distroless}`
- `8.0.0-testnet-{debian|distroless}` and `latest-testnet-{debian|distroless}`
- `8.0.0-staging-{debian|distroless}` and `latest-staging-{debian|distroless}`
...
The version, chain, and flavour are required for all chains. Use an explicit version tag such as `8.0.0-<chain>-<flavour>` for deterministic deployments, or use rolling tags in the form `latest-<chain>-<flavour>`.
...
```bash
sudo docker pull polymeshassociation/polymesh:8.0.0-testnet-distroless
```

The tag `:8.0.0-testnet-distroless` should be replaced with the desired release tag.
...
```bash
sudo docker run --rm polymeshassociation/polymesh:8.0.0-testnet-distroless --version
```

**All available Polymesh [FLAGS], [OPTIONS] and \<COMMAND\> commands can be viewed by calling:**

```bash
sudo docker run --rm polymeshassociation/polymesh:8.0.0-testnet-distroless --help
```
...
```bash
sudo docker run -u root --rm -it polymeshassociation/polymesh:8.0.0-testnet-distroless --chain testnet
```
...
polymeshassociation/polymesh:8.0.0-testnet-distroless \
--chain testnet \
...
This binary is located in `/usr/local/bin/rotate`. In v8, session key rotation requires an ownership proof, so the rotate workflow uses `author_rotateKeysWithOwner` and requires your **stash AccountId** in hex as input (`0x` + 32-byte raw public key bytes).
