...
Both methods deliver to the calling agent. To seize assets and deliver them straight to a third party in a single transaction, use `asset::controller_transfer_to` or `nft::controller_transfer_to` instead, which take a `destination` `AssetHolder` rather than a `destination_kind`. These are useful when returning assets to a rightful owner, since the assets never pass through the agent's own custody.

Both `controller_transfer_to` variants fail if the destination requires the receiver to affirm incoming transfers — a forced transfer cannot bypass that requirement.
...
### Freezing an individual holder

Freezing the whole asset is a blunt instrument. Issuers and agents can instead restrict a single holder — an account key or a portfolio — leaving every other holder unaffected. This is what sanctions screening, dispute holds and per-investor lock-ups need.

- **Freeze a holder:** `asset::set_holder_frozen` with the `asset_holder`, `asset_id` and `freeze` flag. A frozen holder cannot send the asset.
- **Freeze part of a balance:** `asset::freeze_partial_tokens` adds to the amount already frozen, and `asset::unfreeze_partial_tokens` releases some of it. `asset::set_frozen_tokens` sets the frozen amount outright, replacing whatever was frozen before.

A holder's transferable balance becomes `balance − locked − frozen`, so a partially frozen holder can still move whatever is left. Freezing restricts **sending only**: a frozen holder can still receive the asset, and anything they receive is not automatically frozen.

Freezing a holder works for both fungible assets and NFT collections. Freezing **part** of a balance is fungible-only — `asset::set_frozen_tokens`, `freeze_partial_tokens` and `unfreeze_partial_tokens` fail for an NFT collection with `Asset::UnexpectedNonFungibleToken`.

As with asset-wide freezing, controller transfers bypass holder freezes — an agent does not need to unfreeze a holder before seizing from them.
