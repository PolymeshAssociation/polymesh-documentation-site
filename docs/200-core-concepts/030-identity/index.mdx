...
Identity is at the core of Polymesh. All network participants must have an on-chain identity to interact with asset and identity related transactions on the blockchain.
...
- Every Identity related transaction (like basic key management, adding and removing keys and permissioned roles and so on)
- Every Asset related transaction (like holding, minting and distributing an Asset and so on)
- Node operators

An identity is not required for:
- Operations with the POLYX token, like transferring between accounts
- Staking nominators, including operations with stash and controller accounts (like bonding, selecting nominators, unbonding and so on)
