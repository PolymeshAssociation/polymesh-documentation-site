...
In Polymesh, all on-chain actions are associated with both a signing key and an identity, which is created by a [CDD service provider](/identity/verification/). Each identity has associated keys, categorized as primary or secondary, that can sign on its behalf, and each key is linked to only one identity.
...
For a walkthrough on how to create a new MultiSig, please see the [MultiSig Walkthrough](/identity/advanced/multisig).
...
![Child Identity Diagram](./images/identity/child_identities.png)
