...
An asset's issuer or an authorised agent can freeze a portfolio for a **specific asset**, preventing that portfolio from sending it. This is done from the asset side, with `asset::set_holder_frozen`, passing the portfolio as the `asset_holder`. Part of a portfolio's balance can be frozen instead of all of it, using `asset::freeze_partial_tokens`, `asset::unfreeze_partial_tokens` or `asset::set_frozen_tokens`.
