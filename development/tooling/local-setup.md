...
2.  **Configure Environment:** Copy the desired environment file to `.env`. This file sets the Docker image versions and ports. We'll use `envs/8.0`:
    ```bash
    cp envs/8.0 .env
    ```
    *Alternatively, you can specify the env file directly when running compose: `docker compose --env-file=envs/8.0 up`.*
...
*   **`polymesh-rest-api-vault-sm-init`**:
    ...
    *   **Why it's there**: Automates the setup of on-chain accounts and identities corresponding to the keys created in Vault. It waits for the `polymesh-rest-api-vault-sm` service to be healthy, then uses that API to:
        ...
        2.  Use a developer endpoint to make the `admin-1` account a DID registrar on the dev chain.
